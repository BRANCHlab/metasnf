...
build_output_matrix <- function(data_list, design_matrix) {
    ...
    subject_header_df <- as.data.frame(t(as.data.frame(subjects)))
    colnames(subject_header_df) <- subject_header_df[1, ]
    subject_header_df[1, ] <- 0
    output_matrix <- dplyr::inner_join(
        design_matrix,
        subject_header_df,
        by = character())
    return(output_matrix)
