...
execute_design_matrix <- function(data_list, design_matrix) {
    ...
    for (i in seq_len(nrow(design_matrix))) {
        ...
        print(class(fused_network))
        nclusters <- SNFtool::estimateNumberOfClustersGivenGraph(fused_network)
        eigenbest <- nclusters$`Eigen-gap best`
        rotbest <- nclusters$`Rotation cost best`
        eigenisrot <- eigenbest == rotbest
        print(nclusters)
    }
