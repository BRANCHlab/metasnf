...
get_sim_matrix <- function(df, input_type) {
    dist_matrix <- get_dist_matrix(df, input_type)
...
snf_step <- function(data_list, scheme) {
    ...
    if (scheme == "individual") {
        df_list <- lapply(data_list,
            #x[[1]] is the df object, x[[5]] is its input type
            #to-do: give these elements accessible names instead of [[int]]
            function(x) get_sim_matrix(df = x[[1]], input_type = x[[5]]))
    }
    return(df_list)
}
