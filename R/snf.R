...
#' @param race_cd race_cc dataframe generated by `get_*` fns
#' @param race_ed race_ec dataframe generated by `get_*` fns
...
execute_design_matrix <- function(design_matrix,
    ...
    snf_scheme <- "individuals"
    if (snf_scheme == "individuals") {
        if (!(is.null(mtbi_loc))) {
            mtbi_loc_sim <- get_sim_matrix(mtbi_loc)
        }
        if (!(is.null(mtbi_loc))) {
            mtbi_loc_sim <- get_sim_matrix(mtbi_loc)
        }
        if (!(is.null(mtbi_mechanism))) {
            mtbi_mechanism_sim <- get_sim_matrix(mtbi_mechanism)
        }
        if (!(is.null(mtbi_mem_daze))) {
            mtbi_mem_daze_sim <- get_sim_matrix(mtbi_mem_daze)
        }
        if (!(is.null(income))) {
            income_sim <- get_sim_matrix(income)
        }
        if (!(is.null(interview_age))) {
            interview_age_sim <- get_sim_matrix(interview_age)
        }
        if (!(is.null(mtbi_age))) {
            mtbi_age_sim <- get_sim_matrix(mtbi_age)
        }
        if (!(is.null(pubertal_status))) {
            pubertal_status_sim <- get_sim_matrix(pubertal_status)
        }
        if (!(is.null(race_cd))) {
            race_cd_sim <- get_sim_matrix(race_cd)
        }
        if (!(is.null(race_ed))) {
            race_ed_sim <- get_sim_matrix(race_ed)
        }
        if (!(is.null(sex))) {
            sex_sim <- get_sim_matrix(sex)
        }
        if (!(is.null(wmndf))) {
            wmndf_sim <- get_sim_matrix(wmndf)
        }
        if (!(is.null(headaches))) {
            headaches_sim <- get_sim_matrix(headaches)
        }
        if (!(is.null(mtbi_count))) {
            mtbi_count_sim <- get_sim_matrix(mtbi_count)
        }
        if (!(is.null(gord_cor))) {
            gord_cor_sim <- get_sim_matrix(gord_cor)
        }
        if (!(is.null(gord_var))) {
            gord_var_sim <- get_sim_matrix(gord_var)
        }
        if (!(is.null(subc_cor))) {
            subc_cor_sim <- get_sim_matrix(subc_cor)
        }
        if (!(is.null(subc_var))) {
            subc_var_sim <- get_sim_matrix(subc_var)
        }
        if (!(is.null(cort_sa))) {
            cort_sa_sim <- get_sim_matrix(cort_sa)
        }
        if (!(is.null(cort_t))) {
            cort_t_sim <- get_sim_matrix(cort_t)
        }
        if (!(is.null(subc_v))) {
            subc_v_sim <- get_sim_matrix(subc_v, "numeric")
        }
    }
