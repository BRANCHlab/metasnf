...
### A Complex Example: Implementing automated clustering with DBSCAN

Let's say we wanted to cluster our similarity matrices with DBSCAN rather than with spectral clustering.
Recall that we need our function to (1) not require user's to specify the number of clusters AND (2) cluster a *similarity matrix*.
...
Wrapping up their examples into a single function is not easy and will require some compromises.
We can't easily inspect a kNN tree to dial in a sensible eps value.
It's also unclear how practical setting a minPts hyperparameter based on the dimensionality of the data is.
...
So, our function will need to look something like this:

```{r eval = FALSE}
my_dbscan <- function(similarity_matrix) {
    distance_matrix <- as.dist(max(similarity_matrix) - similarity_matrix)
    # the rest of the clustering goes here...
    solution_data <- list("solution" = solution, "nclust" = number_of_clusters)
    return(solution_data)
}
```

Generally, it seems like automating the dbscan is not recommended.

```{r}

iris_dist

kNNdistplot(iris_dist, minPts = 9)

knd <- function (x, k, minPts, ...)
{
...
spl <- smooth.spline(x, y, df = 3)
pred <- predict(spl)
plot(x, y)
lines(pred)
dy <- diff(y)/diff(x)
dpred <- predict(spl, deriv=1)
dpred2 <- predict(spl, deriv=2)
#plot(dpred)
plot(dpred2)
