...
Information about the clustering used by default is stored in the settings matrix:

```{r}
settings_df$"clust_alg"
```
...
You can find this information by running `?settings_df`.
...
To make use of any of these alternative algorithms, you'll need to provide `batch_snf` with a custom `clust_fns_list`.
...
If your custom list is two algorithms long, that's fine, but if it isn't, it's imperative that you either manually adjust the numbers in `settings_df$"clust_algs"` or (more easily) you supply your custom list during settings matrix generation:
...
Then, make sure to provide the `clust_fns_list` once more during the call to `batch_snf`:
