...
### 3. Store all the settings of the desired SNF runs in a *settings_df*

The `settings_df` is a dataframe where each row contains all the information required to convert the raw data into a final cluster solution.
...
An explanation for the parameters in the `settings_df` can be found at [the settings_df vignette](https://branchlab.github.io/metasnf/articles/settings_df.html).
...
The columns in this `settings_df` mean the following:
...
More detailed descriptions on all of these columns can also be found in the settings_df vignette.

### 4. Run SNF

The `batch_snf` function will apply each row of the `settings_df` (in this case, just one row) to the `data_list`.
...
```
The `solutions_matrix` is essentially an augmented `settings_df`, where new columns have been added for each included patient.
...
The subsequent vignettes provide guidance on how you can leverage the `settings_df` to access a wide range of clustering solutions from your data, how you can use other tools in this package to pick a best solution for your purposes, and how to validate the generalizability.
