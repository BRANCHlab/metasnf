...
If you are familiar with alluvial plots and would like to create your own alluvial illustration. `prepare_for_alluvial_wOutcome` function generates a "category frequency" dataframe in wide format that you can use to create your own customized plot. Otherwise, we have ` plotClustersAlluvial_wDiagnosis` to conveniently plot alluvial colored by desired outcome variable. 
...
`Wheeze status` may be just one of the many outcomes that is part of asthma diagnosis. In heterogeneous diseases, "Asthma_Diagnosis" is the ultimate outcome. To display it along side `Wheeze status`:
```{r}
