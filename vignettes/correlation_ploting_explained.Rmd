...
Here's what the heatmap looks like without those transformations:
...
We'll make use of the data stores in

Say I want to display sex annotation on the left, outcome annotations for asthma diagnosis and atopic asthma at the top (for simplicity here let's stick to 2 outcomes):

It may have been the case that

Here is some mock data that is meant to be well separated over gender, asthma diagnosis, and p
...
collapse_dl(data_list) |> colnames()

return_annotation <- function(df,
                              left_bar = NULL,
                              right_bar = NULL,
                              top_bar = NULL,
                              bottom_bar = NULL,
...
annotations_list <- return_annotation(
    merged_df,
    left_hm = list(
        "Gender" = "gender"
    ),
...
ith_annotation <- ComplexHeatmap::HeatmapAnnotation(
    temporary_name  = anno_barplot(merged_df[, "age"])
)
names(ith_annotation@anno_list) <- "newname"
ith_annotation@anno_list[[1]]@name <- "newname"
ith_annotation@anno_list[[1]]@label <- "newname"
...
top_ha <- ComplexHeatmap::HeatmapAnnotation(
    #Age = anno_barplot(merged_df$"age"),
...
Do take a look at the [ComplexHeatmap Complete Reference](https://jokergoo.github.io/ComplexHeatmap-reference/book/index.html) to learn more about what is possible with this package.
...
Otherwise, we have `plotClustersAlluvial_wDiagnosis` to conveniently plot alluvial colored by desired outcome variable.
...
plotClustersAlluvial_wDiagnosis(
    similarity_matrix = similarity_matrix,
    df_env = df_outcomes,
    numc = 6,
