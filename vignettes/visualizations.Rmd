...
df_outcomes = data.frame(
    ...
    Asthma_Diagnosis = c( rep("Definite Asthma", 10), rep("Possible Asthma", 50), rep("No Asthma", 140))
)
...
alluvial_cluster_plot <- function(cluster_sequence,
    ...
    key_outcome_df <- data[, colnames(data) %in% c("subjectkey", key_outcome)]
    ...
    alluvial_df <- alluvial_df |>
        ...
        data.frame()
    alluvial_indices <- 1:(length(cluster_sequence) + length(key_outcome))
    ...
    ggplot2::labs(title = key_outcome) +
    ggplot2::theme_bw() +
    ggplot2::theme(
