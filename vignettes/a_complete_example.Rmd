...
- **Component dropout** - Information on whether or not a particular input data frame in the data list will be in included or excluded from the corresponding SNF run (discussed further in the [settings matrix vignette](https://branchlab.github.io/metasnf/articles/settings_df.html)).
...
More on `settings_df` customization can be found in the [settings matrix vignette](https://branchlab.github.io/metasnf/articles/settings_df.html).
...
# Working on extracting solutions data frame properly
sol_df[1]
...
extended_sol_df <- extend_solutions(
    sol_df,
    target_dl = target_dl
)

# What columns have been added?
old_cols <- colnames(extended_sol_df) %in% colnames(sol_df)
new_cols <- !old_cols

head(extended_sol_df[, new_cols])
...
This `extended_sol_df` we created can be passed into the `adjusted_rand_index_heatmap` function to easily visualize the level of separation on each of our features for each of our cluster solutions.
...
annotated_ari_hm <- adjusted_rand_index_heatmap(
    ...
    annotation_colours = list(
        "Depression p-value" = colour_scale(
            extended_sol_df$"cbcl_depress_r_pval",
            min_colour = "purple",
            max_colour = "black"
        ),
        "Anxiety p-value" = colour_scale(
            extended_sol_df$"cbcl_anxiety_r_pval",
            min_colour = "green",
            max_colour = "black"
        ),
        "Overall outcomes p-value" = colour_scale(
            extended_sol_df$"mean_pval",
...
```{r eval = FALSE}
extended_sol_df2 <- extended_sol_df |>
    dplyr::mutate(
...
Recall that the `sol_df` (and, by extension, the `extended_sol_df`) is an extension of the settings matrix, so no further data manipulation is needed to add a setting as a heatmap annotation.
...
sol_df <- batch_snf_results$"sol_df"
similarity_matrices <- batch_snf_results$"similarity_matrices"
```

This time, the output of `batch_snf` is a list.
The first element of the list is a single sol_df, like what we usually get.
...
```{r}
extended_sol_df <- extend_solutions(sol_df, target_list)

colnames(extended_sol_df)[1:25]

# Looking at the newly added columns
head(no_subs(extended_sol_df))
```

If you just want the p-values:

```{r}
target_pvals <- get_pvals(extended_sol_df)
...
all_subjects <- input_dl[[1]]$"data"$"uid"
...
extended_sol_df |> colnames()

# The fifth row had the lowest minimum p-value across our outcomes
lowest_min_pval <- min(extended_sol_df$"min_pval")
which(extended_sol_df$"min_pval" == lowest_min_pval)

# Keep track of your top solution
top_row <- extended_sol_df[4, ]
...
propagated_labels <- lp_sol_df(top_row, full_input_dl)
