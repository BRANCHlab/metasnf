...
Let's say we are working in a context where the anxiety and depression data are especially important outcomes and we want to know if we can find subtypes using the other data which still do a good job of separating out subjects by their anxiety and depression scores.
...
Upon data list creation, the UID will be converted to "uid" for consistency across other functions in the package.

We can get a summary of our constructed `data_list` with the `summary` function:

```{r}
summary(input_dl)
```

Each input data frame now has the same 87 subjects with complete data.
...
The SNF config stores all the information about the settings we'd like to use for each of our SNF runs.
...
- **Component dropout** - Information on whether or not a particular input data frame in the data list will be in included or excluded from the corresponding SNF run (discussed further in the [settings data frame vignette](https://branchlab.github.io/metasnf/articles/settings_df.html)).
...
Each row of the settings data frame corresponds to a set of hyperparameters for generating a final cluster solution.
...
Further customization of the `settings_df` and the other parts of the SNF config will enable you to generate a broader space of cluster solutions from your data and ideally get you closer to finding a more generalizable and useful solution for your context.
More on `settings_df` customization can be found in the [settings data frame vignette](https://branchlab.github.io/metasnf/articles/settings_df.html).

Setting a seed prior to calling `snf_config()` ensures that the same settings data frame is generated each time we run the code.

While we end up with a random set of settings here, there is nothing wrong with manually altering the settings data frame to suit your needs.
...
There are three other parts to an SNF config object: a distance functions list (`dist_fns_list` class object) that stores distance metric functions, a clustering functions list (`clust_fns_list` class object) that stores clustering algrithm functions, and a weights matrix (`weights_matrix`) class object that stores feature weights.
...
- [distance functions list](https://branchlab.github.io/metasnf/articles/dist_fns_list.html)
- [clustering functions list](https://branchlab.github.io/metasnf/articles/clust_fns_list.html)
- [weights matrix](https://branchlab.github.io/metasnf/articles/weights_matrix.html)

For now, we'll leave them as the default values generated during our `snf_config` call.

## Running SNF and clustering

The `batch_snf()` function integrates the data in the data list using the settings contained in the SNF config.
The resulting structure is an `solutions_df` class object, which contains an index column (`solution`), a column tracking the number of clusters in the cluster solution (`nclust`), and columns specifying which cluster each subject was assigned to for each corresponding `settings_df` row.
...
When transposed, the `solutions_df` class object is converted to a `t_solutions_df` class object.
It is like a regular transposition, but the `solution` and `nclust` columns are dropped.
...
save_heatmap(
    heatmap = ari_hm,
    path = "vignettes/adjusted_rand_index_heatmap.png",
    width = 330,
    height = 300,
    res = 100
)
...
This heatmap is integral to understanding what meta clusters exist in your data (given the space of parameters you explored in the settings data frame).
...
You can determine  vector through trial and error or by using the `shiny_annotator` function (a wrapper around functionality from the `InteractiveComplexHeatmap` package).
...
# Summary columns of target_dl features tacked on the end
ext_sol_df[, (ncol(ext_sol_df) - 2):ncol(ext_sol_df)]
```
...
# Also would work, but now every feature used to calculate summaries
ext_sol_df <- extend_solutions(
    sol_df,
...
This `ext_sol_df` we created can be converted into a standard data frame and passed into the `adjusted_rand_index_heatmap` function to easily visualize the level of separation on each of our features for each of our cluster solutions.
...
```{r eval = FALSE}
# Note that removing columns from an `ext_solutions_df` class object turns
# it into a regular data frame.
...
save_heatmap(
    heatmap = sm_hm,
    path = "./settings_df_heatmap_ordered.png",
    width = 400,
    height = 500,
    res = 75
)
...
<center>
![](https://raw.githubusercontent.com/BRANCHlab/metasnf/main/vignettes/settings_df_heatmap_ordered.png)
</center>
...
all_subjects <- uids(input_dl)

all_subjects

# Remove the "uid_" prefix to allow merges with the original data
all_subjects <- gsub("uid_", "", all_subjects)
...
# Use the solutions data frame from the training subjects and the data list from
# the training and testing subjects to propagate labels to the test subjects
propagated_labels <- lp_sol_df(top_row, full_dl)
...
```{r}
propagated_labels_all <- lp_sol_df(
    ext_sol_df,
    full_dl
)
