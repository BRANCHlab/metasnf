...
By converting an `ext_solutions_df` class object into a data frame, it'll retain both the p-value and cluster assignment columns.
...
```{r}
ext_sol_df <- extend_solutions(sol_df, target_list)

colnames(ext_sol_df)[1:25]

# Looking at the newly added columns
head(no_subs(ext_sol_df))
```

If you just want the p-values:

```{r}
target_pvals <- get_pvals(ext_sol_df)
