...
The width refers to the number of columns in each dataframe, which equals 1 for the UID (uid) column + the number of features in the dataframe.
...
The `settings_df` stores all the information about the settings we'd like to use for each of our SNF runs.
Calling the `settings_df` function with a specified number of rows will automatically build a randomly populated `settings_df`.
...
- `inc_*` - binary columns indicating whether or not an input dataframe is included (1) or excluded (0) from the corresponding SNF run (discussed further in the [settings matrix vignette](https://branchlab.github.io/metasnf/articles/settings_df.html))

Without specifying any additional parameters, `settings_df` randomly populates these columns and ensures that no generated rows are identical.
...
Further customization of the `settings_df` will enable you to access the broadest possible space of reasonable cluster solutions that your data can produce using SNF and ideally get you closer to a generalizable and useful solution for your context.
...
You can also add random rows to an already existing dataframe using the `add_settings_df_rows` function (further discussed in the vignette).

## Running SNF and clustering

The `batch_snf` function integrates the data in the `data_list` using each of the sets of settings contained in the `settings_df`.
The resulting structure is an `solutions_matrix` which is an extension of the `settings_df` that contains columns specifying which cluster each subject was assigned for the corresponding `settings_df` row.
...
all_subjects <- data_list[[1]]$"data"$"uid"
