...
At this point, we have our meta clusters but are not yet sure of how they differ in terms of their structure across the input variables.
To get a quick overview of which meta cluster we want to further explore, let's determine how good these meta clusters were at separating apart our patients over their anxiety and depression scores.

We start by running the `extend_solutions` function, which will calculate p-values representing the strength of the association between cluster membership (treated as a categorical variable) and any feature present in a provided target list.
...
new_cols <- !colnames(extended_solutions_matrix) %in%
    colnames(solutions_matrix)
head(extended_solutions_matrix[, new_cols])
```

The `extended_solutions_matrix` can be passed into the `adjusted_rand_index_heatmap` function to easily visualize the level of separation on each of our variables for each of our cluster solutions.
...
This heatmap and its annotations is perhaps the richest visualization offered in this package for rapidly understanding the sorts of cluster solutions you've generated over a wide range of settings.
...
It may be useful to get a quick view of how the original input variables separated for each of the generated cluster solutions as well.
