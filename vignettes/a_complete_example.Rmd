...
Some of our data has missing values and not all of our data frames have the same number of participants.
...
# Reducing data frames to only common subjects with no missing data
...
This heatmap is integral to understanding what meta clusters were created from your data using the space of parameters you defined in the SNF config.
...
Which can be used to populate the meta cluster column in our solutions data frame as well as visualize our meta cluster boundaries more clearly.
...
z1 <- get_representative_solutions(
    sol_aris,
    split_vector = split_vec,
    order = meta_cluster_order,
    ext_sol_df
...
z2 <- get_representative_solutions2(
    sol_aris,
    split_vector = split_vec,
    order = meta_cluster_order,
    ext_sol_df
...
```{r}
# All the subjects present in all data frames with no NAs
...
# data frame assigning 80% of subjects to train and 20% to test
