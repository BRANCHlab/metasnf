...
generate_assists <- function(level, position) {
    ...
    assists <- rnorm(n = 1, mean = 1000, sd = 30)
    # Effect of playing in the NBA
    if (level == "NBA") {
        assists <- assists + rnorm(n = 1, mean = 2500, sd = 20)
    } else {
        # Match the noisiness of the NBA players
        assists <- assists + rnorm(n = 1, mean = 0, sd = 20)
    }
    # Effect of the player's position
    if (position == "pg") {
        assists <- assists + 300 # Just to be explicit about it
    } else if (position == "c") {
        assists <- assists + 0
    } else if (position == "sg") {
        assists <- assists + 150
...
#
#
available_data <- player_data |> dplyr::select("id", "assists", "blocks")
#
...
```{r}
player_data$"id" <- 1:nrow(player_data)

player_data_df <- player_data |>
    dplyr::select("id", "blocks", "assists")

player_data_df_no_id <- player_data |>
    dplyr::select("blocks", "assists")

distance <- as.matrix(dist(player_data_df))

clusters <- SNFtool::spectralClustering(SNFtool::affinityMatrix(distance), K = 6)
