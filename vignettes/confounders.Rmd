...
# Plot by position
player_data |>
    ggplot(aes(x = adjusted_blocks, y = adjusted_assists, shape = level, colour = position)) +
    geom_point(size = 5, alpha = 0.3) +
    theme_bw()

# Too much adjustment:
# player_data$"adjusted_blocks2" <- resid(lm(blocks ~ level + position, player_data))
# player_data$"adjusted_assists2" <- resid(lm(assists ~ level + position, player_data))
#
# player_data |>
#     ggplot(aes(x = adjusted_blocks2, y = adjusted_assists2, shape = level, colour = position)) +
#     geom_point(size = 5, alpha = 0.3) +
#     theme_bw()
...
player_data2$"adjusted_blocks" <- NULL
player_data2$"adjusted_assists" <- NULL
...
A <- Map(
    function(i,j) {
        ...
        results <- summary(lm(formula, data = mtcars))
        results$coef[, c(1, 4)]
    },
    rep(
        y,
...
When you remove the signal associated with a variable, you will impact every other variable to a degree that is proportional to the correlation between that variable and the unwanted one.
Practically, any variable that is strongly correlated with your unwanted signal variable may be better off removed.

Consider two variables: temperature and ice cream sales.
