...
The `design_matrix` stores all the information about the settings we'd like to use for each of our SNF runs.
Calling the `generate_design_matrix` function with a specified number of rows will automatically build a randomly populated `design_matrix`.

```{r}
#design_matrix <- generate_design_matrix(data_list, nrow = 30)
design_matrix <- generate_design_matrix(data_list, nrow = 5, seed = 42)
...
`generate_design_matrix` randomly populates these columns and ensures that no generated rows are identical.
Setting the optional `seed` parameter (which will affect the seed of your entire R session) ensures that the same design matrix is generated each time we run the code.

While we end up with a random set of settings here, there is nothing wrong with manually altering the design matrix to suit your needs.
...
You can also add random rows to an already existing dataframe using the `add_design_matrix_rows` function.


## Executing the design matrix

The `execute_design_matrix` function integrates the data in the `data_list` using each of the sets of settings contained in the `design_matrix`.
The resulting structure is an `output_matrix` which is an extension of the `design_matrix` that contains columns specifying which cluster each subject was assigned for the corresponding `design_matrix` row.

```{r}
output_matrix <- execute_design_matrix(data_list, design_matrix)

colnames(output_matrix)

output_matrix[1:5, 1:20]
```
...
We now have a group of cluster solutions stored in an `output_matrix`.
Proposing tens or hundreds (or thousands) of clustering solutions in a research paper doesn't seem to be in-style right now, so we will need to pick a solution to move forward with.
...
I believe this approach is best for truly exploratory clustering (read: when you have some sort of vague vibes about when one solution is better than another) and it is difficult to specify exactly what you are looking for.

```{r}
# Only row_id and the subject columns
subs(output_matrix)[1:5, 1:10]

colnames(output_matrix)

output_matrix_aris <- calc_om_aris(output_matrix)

output_matrix_aris

ari_heatmap(output_matrix_aris)

# You can optionally save the heatmap with the line below
# ari_heatmap(output_matrix_aris, save = "./ari_heatmap.png")
```
...
The [adjusted Rand index](https://en.wikipedia.org/wiki/Rand_index) (ARI) is a tool for helping us calculate similarities between clustering solutions.
...
Agglomerative hierarchical clustering is being applied to these solutions by default (thank you, `pheatmap` package) so the orders of the clustering solutions here do not exactly line up with the order of the clustering solutions present in the design matrix.
...
```{r echo = FALSE, fig.align = 'center'}
ari_heatmap(output_matrix_aris, save = "./ari_heatmap_unclustered.png")
```

![.](./ari_heatmap_unclustered.png){width=400px}

And there's an option to hide the corresponding design matrix rows:

```{r}
ari_heatmap(output_matrix_aris, hide_ids = TRUE)
```

If you see something interesting in your heatmap, you may be curious to know how that corresponds to the settings that were in your design matrix.

I recommend visualizing the design matrix with yet another heatmap:

```{r}
dm_heatmap(design_matrix)
```

You can use the solution order in the clustered ARI heatmap to reorder the design matrix heatmap:

```{r}
om_ari_order <- get_pheatmap_order(output_matrix_aris)

om_ari_order

dm_heatmap(design_matrix, order = om_ari_order)
```

Maybe you'll see something interesting!

With more design matrix rows and perhaps a less random dataset, you may end up seeing some proper clusters-of-clusters appearing.

If there's a row that interest you (say, row 5), this format may be more useful for your subsequent analyses:

```{r}
cluster_df <- get_cluster_df(output_matrix[5, ])
...
**Warning: This approach can very easily result in [overfitting](https://en.wikipedia.org/wiki/Overfitting) your data and producing clustering results that generalize poorly to subjects outside of your dataset. Set aside some subjects to validate your results to avoid this issue.** 
...
The `outcome_list` contains dataframes what we can examine our clustering results over through linear or ordinal regression.
Categorical data hasn't been added yet.
...
We can use it to extend our `output_matrix` with p-values from regressing the `outcome_list` features onto our generated clusters.

```{r}
extended_output_matrix <- extend_om(output_matrix, outcome_list)

extended_output_matrix
```

If you just want the p-values:

```{r}
p_val_select(extended_output_matrix)
```

Please do not interpret these p-values in the typical hypothesis-testing way.
In this context, they are just a tool to find clustering solutions that are well-separated according to the outcome measures you've specified.
Finding a cluster solution like this is a bit of a supervised learning problem, but where the optimization method is just random sampling.
The risk for overfitting your data with this approach is large, so make sure you have some rigorous external validation before reporting your findings.
...
This package provides a wrapper for `SNFtool::groupPredict` to make the label propagation process a little easier to apply to the `output_matrix` structure.
...
train_abcd_cort_t <- keep_split(abcd_cort_t, assigned_splits, "train")
train_abcd_cort_sa <- keep_split(abcd_cort_sa, assigned_splits, "train")
train_abcd_subc_v <- keep_split(abcd_subc_v, assigned_splits, "train")
train_abcd_income <- keep_split(abcd_income, assigned_splits, "train")
train_abcd_pubertal <- keep_split(abcd_pubertal, assigned_splits, "train")
train_abcd_anxiety <- keep_split(abcd_anxiety, assigned_splits, "train")
train_abcd_depress <- keep_split(abcd_depress, assigned_splits, "train")

# Partition a test set
test_abcd_cort_t <- keep_split(abcd_cort_t, assigned_splits, "test")
test_abcd_cort_sa <- keep_split(abcd_cort_sa, assigned_splits, "test")
test_abcd_subc_v <- keep_split(abcd_subc_v, assigned_splits, "test")
test_abcd_income <- keep_split(abcd_income, assigned_splits, "test")
test_abcd_pubertal <- keep_split(abcd_pubertal, assigned_splits, "test")
test_abcd_anxiety <- keep_split(abcd_anxiety, assigned_splits, "test")
test_abcd_depress <- keep_split(abcd_depress, assigned_splits, "test")
...
## Note, the training_subjects and testing_subjects parameters are necessary
## to construct the full_data_list in the right subject order for
## label propagation.
...
design_matrix <- generate_design_matrix(train_data_list, nrow = 5, seed = 42)

train_output_matrix <- execute_design_matrix(train_data_list, design_matrix)

extended_output_matrix <- extend_om(train_output_matrix, train_outcome_list)

# The fourth row had the lowest minimum p-value across our outcomes
which(extended_output_matrix$"min_p_val" ==
      min(extended_output_matrix$"min_p_val"))

# Keep track of your top solution
top_om_row <- extended_output_matrix[4, ]

# Hand over the output matrix as well as the full data list to propagate labels
#  to the test subjects
propagated_labels <- lp_om(top_om_row, full_data_list)
head(propagated_labels)

# You could, in theory, see how all of your clustering solutions propagated,
#  but that would mean reusing your validation set and removing the protection
#  against overfitting conferred by this procedure.
propagated_labels_all <- lp_om(extended_output_matrix, full_data_list)
...
This section describes how the `generate_design_matrix` function randomly assigns input dataframess to be dropped from different SNF runs to access a wider space of possible solutions.

By default, `generate_design_matrix` will build an empty dataframe containing the columns outlined in the [design matrix][Generating the design matrix] section.
If a non-zero number of rows are specified when calling `generate_design_matrix`, the function calls on `add_design_matrix_rows` to add rows with randomly valid values.
