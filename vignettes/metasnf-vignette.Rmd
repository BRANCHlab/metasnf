...
unique(summarize_dl(data_list)$"length")

max(settings_matrix$"k")
...
The `target_list` contains dataframes what we can examine our clustering results over through linear regression (numeric data), ordinal regression (ordinal data), or the Chi-squared test (categorical data).
...
```{r}
extended_solutions_matrix <- extend_om(solutions_matrix, target_list)
...
extended_solutions_matrix <- extend_om(
