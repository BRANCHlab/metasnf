...
By default, `batch_snf` will create its own `distance_metrics_list` by calling the `distance_metrics_list` function with no additional arguments.

```{r}
library(metasnf)

distance_metrics_list <- distance_metrics_list(use_default_dist_fns = TRUE)
```
...
```{r}
my_distance_metrics <- distance_metrics_list(
    cnt_dist_fns = list(
...
The first way is to use the `keep_defaults` parameter in `distance_metrics_list()`:

```{r}
no_default_metrics <- distance_metrics_list(
    cnt_dist_fns = list(
        "standard_norm_euclidean" = sn_euclidean_distance
    ),
    dsc_dist_fns = list(
